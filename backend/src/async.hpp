...
namespace detail {
    ...
    struct apply_impl {
        static void apply(F f, Tuple && t) {
            apply_impl<F, Tuple, Total == 1 + sizeof...(N), Total, N...,
                       sizeof...(N)>::apply(f, std::forward<Tuple>(t));
        }
    };
...
template <typename F, typename Tuple> void apply(F f, Tuple && t) {
    typedef typename std::decay<Tuple>::type ttype;
    detail::apply_impl<
        F, Tuple, 0 == std::tuple_size<ttype>::value,
        std::tuple_size<ttype>::value>::apply(f, std::forward<Tuple>(t));
}

struct async {
    template <typename F, typename... Args> struct work {
    ...
    static void start(const js_cb & callback, F && work_routine,
                      Args &&... args) {
        ...
        uv_queue_work(
            uv_default_loop(), &task->m_request,
            [](uv_work_t * req) {
